...
try {
    ...
    echo "$(date) PostSysprep.ps1 starting" > $env:SystemDrive\packer\PostSysprep.txt

    #--------------------------------------------------------------------------------------------

    # Download and install Cygwin for SSH capability
    echo "$(date) PostSysprep.ps1 downloading cygwin..." > $env:SystemDrive\packer\PostSysprep.txt
    ...
    echo "$(date) PostSysprep.ps1 installing cygwin..." >> $env:SystemDrive\packer\PostSysprep.txt
    ...
    # Activate the VM
    echo "$(date) PostSysprep.ps1 activating..." >> $env:SystemDrive\packer\PostSysprep.txt
    ...
    echo "$(date) PostSysprep.ps1 downloading NSSM configuration file..." >> $env:SystemDrive\packer\PostSysprep.txt
    ...
    echo "$(date) PostSysprep.ps1 downloading NSSM..." >> $env:SystemDrive\packer\PostSysprep.txt
    $wc=New-Object net.webclient;$wc.Downloadfile("https://nssm.cc/release/nssm-2.24.zip","$env:Temp\nssm.zip")
    echo "$(date) PostSysprep.ps1 extracting NSSM..." >> $env:SystemDrive\packer\PostSysprep.txt
    Expand-Archive -Path $env:Temp\nssm.zip -DestinationPath $env:Temp
    echo "$(date) PostSysprep.ps1 installing NSSM..." >> $env:SystemDrive\packer\PostSysprep.txt
    Copy-Item $env:Temp\nssm-2.24\win64\nssm.exe $env:SystemRoot\System32
    
    
    # Configure the docker NSSM service
    echo "$(date) PostSysprep.ps1 configuring NSSM..." >> $env:SystemDrive\packer\PostSysprep.txt
    ...
    echo "$(date) PostSysprep.ps1 resetting networking (hack)..." >> $env:SystemDrive\packer\PostSysprep.txt
    netsh int ipv4 reset
    
    #--------------------------------------------------------------------------------------------
    
    echo "$(date) PostSysprep.ps1 configuring temp to D..." >> $env:SystemDrive\packer\PostSysprep.txt
    $env:Temp="d:\temp"
    $env:Tmp=$env:Temp
...
Catch [Exception] {
    echo "$(date) PostSysprep.ps1 complete with Error '$_'" >> $env:SystemDrive\packer\PostSysprep.txt
    exit 1
}
Finally {

    # Disable the scheduled task
    echo "$(date) PostSysprep.ps1 deleting scheduled task.." >> $env:SystemDrive\packer\PostSysprep.txt
    $ConfirmPreference='none'
    Get-ScheduledTask 'PostSysprep' | Disable-ScheduledTask

    echo "$(date) PostSysprep.ps1 rebooting..." >> $env:SystemDrive\packer\PostSysprep.txt
    ...
    echo "$(date) PostSysprep.ps1 complete successfully at $(date)" >> $env:SystemDrive\packer\PostSysprep.txt
}    